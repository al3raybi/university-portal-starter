 ...
 * - 84 students (Exactly 7 per department)
 * - ~500 enrollments (each student takes several courses; many are graded)
 *
...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ---- Students (Exactly 7 per department = 84 total) ----
        $studentRows = [];
        $s = 1;
        foreach ($departments as $d) {
            for ($i = 0; $i < 7; $i++) { // Changed from 20 to 7 to make total 84
                $fn = $firstNames[array_rand($firstNames)];
                $ln = $lastNames[array_rand($lastNames)];
                $studentRows[] = [
                    'name' => "{$fn} {$ln}",
